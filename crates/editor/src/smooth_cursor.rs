...
impl SmoothCursorAnimationState {
    ...
    pub(crate) fn step(
        ...
        let trail = if settings.trail && settings.trail_opacity > 0.0 && animating {
            let max_dist = (0..4)
                .map(|i| {
                    let dx = target_corners[i].x.as_f32() - self.corners[i].x.as_f32();
                    let dy = target_corners[i].y.as_f32() - self.corners[i].y.as_f32();
                    (dx * dx + dy * dy).sqrt()
                })
                .fold(0.0f32, f32::max);
            ...
            if max_dist > settings.trail_min_distance {
                let visibility_ratio =
                    (max_dist / (self.target_bounds.size.height.as_f32() * 0.5)).clamp(0.0, 1.0);
